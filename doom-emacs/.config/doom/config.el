...
(setq doom-font (font-spec :family "JetBrains Mono NL" :size 18)
      ...
      doom-variable-pitch-font (font-spec :family "JetBrains Mono NL" :size 18))
...
;; Custom heading text font sizes
(after! org
  (custom-set-faces!
...
(use-package! copilot
  :config
  ;; Delay copilot activation to avoid conflicts with syntax highlighting
  (defun my/enable-copilot-after-delay ()
  ...
  (add-hook 'prog-mode-hook #'my/enable-copilot-after-delay)
...
(after! org
  ...
  (org-babel-do-load-languages
   ...
   '((emacs-lisp . t)     ; Emacs Lisp
     ...
     (awk . t)            ; awk
     (calc . t)           ; Calc
     (ditaa . t))))       ; ditaa diagrams - ADDED MISSING CLOSING PAREN
